...
# (Imports, constants, model definitions remain the same)
import feedparser
import numpy as np
import torch
...
# from recommendations.ml_models import AvgPostToAccountModel # Assume defined below
...
class ResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, dropout):
        super().__init__()
        self.block = nn.Sequential(nn.Linear(input_dim, output_dim), nn.LayerNorm(output_dim), nn.ReLU(), nn.Dropout(dropout))
...
class AvgPostToAccountModel(nn.Module):
    def __init__(self, input_dim, output_dim, dropout=0.2):
        ...
        self.model = nn.Sequential(ResidualBlock(input_dim, 1024, dropout), ResidualBlock(1024, 512, dropout), ResidualBlock(512, 256, dropout), nn.Linear(256, output_dim))
    def forward(self, x): return self.model(x)
# ------------------------------

class Command(BaseCommand):
    help = 'Uses AvgPostToAccountModel to map news embedding and find similar bot.'
    # ... (__init__, get_device, load_avg_post_to_account_model, get_bot_user_objects, fetch_rss_summaries, get_openai_embedding, map_openai_to_account_vector, find_most_similar_bot unchanged) ...
    ...
    def load_avg_post_to_account_model(self):
        model = AvgPostToAccountModel(input_dim=OPENAI_DIM, output_dim=NODE2VEC_DIM)
        try:
            model.load_state_dict(torch.load(AVG_POST_TO_ACCOUNT_MODEL_PATH, map_location=self.device)); model = model.to(self.device); model.eval()
            self.stdout.write(self.style.SUCCESS(f"Loaded AvgPostToAccountModel from {AVG_POST_TO_ACCOUNT_MODEL_PATH}"))
            return model
        except FileNotFoundError: self.stdout.write(self.style.ERROR(f"Model not found: {AVG_POST_TO_ACCOUNT_MODEL_PATH}")); return None
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if valid_vector_count < required_bot_count:
                ...
                # ★★★ Linter Error箇所 修正: ここで return するのが正しい ★★★
                return
            # ★★★ Linter Error箇所 修正: elseブロックを正しく記述 ★★★
            else:
                 self.stdout.write(self.style.SUCCESS("All required bot accounts have Node2Vec embeddings."))
                 # ここで処理を止めずに続ける (インデント注意)

        # ★★★ これ以降の処理は try ブロックや else ブロックの外 (handleメソッド直下) ★★★
